...
todo_client.todo_items.attachments.create_json_attachment(
    todo_item.id, TodoAttachment(filename="test2.jpg", media_type="image/jpeg", contents=b"test")
)
todo_client.todo_items.attachments.create_file_attachment(
    todo_item.id, FileAttachmentMultipartRequest(contents=open(image_path, "rb"))
)
